...
// ==========================
// CRIAR PIX — SHARKBANKING
// ==========================
async function createPix(chatId, amountReais) {
  const amountCents = Math.round(amountReais * 100);
  const orderId     = `shark_${chatId}_${Date.now()}`;
  const auth        = 'Basic ' + Buffer.from(`${process.env.SHARKBANKING_PUBLIC_KEY}:${process.env.SHARKBANKING_SECRET_KEY}`).toString('base64');
  ...
  const payload = {
    ...
    customer,
    externalRef:   orderId,
    postbackUrl:   process.env.SHARKBANKING_POSTBACK_URL,
    items: [{
  ...
  console.log(`📤 [SharkBanking] Criando PIX R$ ${amountReais.toFixed(2)} | chatId: ${chatId}`);
  ...
  console.log(`✅ [SharkBanking] PIX gerado | orderId: ${orderId}`);
